...
import { getIngredients as getIngredientsAction, decreaseIngredient as decreaseIngredientAction, addIngredient as addIngredientAction } from 'redux/actions/orderFoodAction'
...
const Ingredients = ({
  ...
  return (
    <div>
      {
        isLoading && (
          <div className="skeleton-0d1e66qm5fq"></div>
        )
      }
      {
        error && (
            ...
            <li key={elem.ing}>
              <div className="flex items-center bg-gray-200 p-3 rounded-lg hover:bg-gray-300">
                ...
                <div className="ml-auto">
                  <Counter value={ingredientsSelected?.[elem.ing]?.quantity ?? 0} onChangeValue={(operation) => changeValueElement(operation, elem)} />
                </div>
              </div>
            </li>          
            ))
      ...
      <button disabled={conditionToSave || isLoading} onClick={() => onClickButtonOrder && onClickButtonOrder()} className="disabled:opacity-50 w-full text-gray-50 text-lg bg-yellow-400 hover:bg-yellow-300 focus:outline-none focus:ring-2 focus:ring-yellow-600 focus:ring-opacity-50 rounded-lg py-2">
        Order
      </button>
      {
        conditionToSave && (
          <span className="text-xs text-gray-500">You must add at least 15 ingredients</span>
        )
      }
    </div>
  )
